...
class HashMap {
    ...
    insert(key, value) {
        ...
        // Check if the load factor will exceed the threshold after insertion
        ...
        while (temp != null) {
            if (temp.key === key) {
                temp.value = value; // Return the value if key is found
                return
            }
    ...
    search(key) {
        const bucketIndex = this.hashFunction(key); // Get the index in the array using hash function
        let temp = this.arr[bucketIndex].head; // Get the head of the linked list at that index
